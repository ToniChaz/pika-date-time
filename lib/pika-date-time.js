...
/*
 * Pikadatetime
 * Copyright © 2015 Toni Chaz | MIT license | http://www.tonichaz.com/
 */
...
(function (root, define, factory) {
  ...
  var hasMoment = typeof moment === 'function',
    ...
    extend = function (to, from, overwrite) {
      ...
      for (prop in from) {
        ...
        if (hasProp && typeof from[prop] === 'object' && from[prop].nodeName === undefined) {
          if (isDate(from[prop]) && overwrite) {
            to[prop] = new Date(from[prop].getTime());
          } else if (isArray(from[prop]) && overwrite) {
    ...
    defaults = {
      ...
      // close when outside of calendar click
      autoClose: true,
      ...
      lang: 'en', // The initial language selected
    ...
    renderTitle = function (instance) {
      ...
      html += '<button class="pika-close" type="button">' + opts.i18n[opts.lang].close + '</button>';
    ...
    renderTimePicker = function (numOptions, selectedVal, selectClass, displayFunc) {
      var toReturn = '<td><select class="pika-select ' + selectClass + '">';
      for (var i = 0; i < numOptions; i++) {
        toReturn += '<option value="' + i + '" ' + (i === selectedVal ? 'selected' : '') + '>' + displayFunc(i) + '</option>';
      }
      toReturn += '</select></td>';
      return toReturn;
    },
    ...
    renderTime = function (hh, mm, ss, opts) {
      var toReturn = '<table cellpadding="0" cellspacing="0" class="pika-time"><tbody><tr>' +
        renderTimePicker(24, hh, 'pika-select-hour', function (i) {
          if (opts.use24hour) {
            ...
            var toReturn = (i % 12) + (i < 12 ? ' AM' : ' PM');
            if (toReturn === '0 AM') {
              return opts.i18n[opts.lang].midnight;
            } else if (toReturn === '0 PM') {
              return opts.i18n[opts.lang].noon;
            } else {
              return toReturn;
            }
          }
      ...
      return toReturn + '</tr></tbody></table>';
    },
    ...
    Pikaday = function (options) {
      ...
      self._onClick = function (e) {
        ...
        if (opts.autoClose && self._v && target !== opts.trigger) {
          self.hide();
        }
      };
  ...
  Pikaday.prototype = {
    ...
    draw: function (force) {
      ...
      if (opts.showTime) {
        this.el.innerHTML += renderTime(this._hh, this._mm, this._ss, this._o);
      }
    ...
    adjustPosition: function () {
      ...
      if (typeof field.getBoundingClientRect === 'function') {
       ...
       if (left + width > viewportWidth) {
        left = left + field.offsetWidth - width;
      }
